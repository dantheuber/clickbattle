/**
 * Module for initializing the game api request parameters for /api/games routes.
 * Export the {@link game:Parameters~registerGameParams}
 * function to register the api routes on the passed express router.
 * @module {function} game:parameters
 * @requires {@link game:model}
 */
'use strict';

// export the function to register all game request params
module.exports = registerGameParams;

/**
 * Attach request parameters to the given router.
 * @param router {express.Router} - The router to attach the parameters to
 */
function registerGameParams(router) {
	// router.param('id', registerParamName);
	// add params below
}

/*
 * Register a parameter for /api/games requests.
